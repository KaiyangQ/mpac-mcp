...
_SYSTEM_PROMPT = (
    ...
    "is a protocol violation. After EVERY defer_intent call, inspect "
    "its response. If it has `must_retry_announce: true` (or "
    "`status: resolved` with `reason: observed_intents_terminated`), "
    "the intent you yielded to is already gone; DO NOT tell the user "
    "you are waiting or that you yielded. Immediately retry by calling "
    "check_overlap/announce_intent for the same files, then continue "
    ...
    "withdraw.\" Do NOT write files in this branch. If the response "
    "includes `duplicate_candidate`, mention that the other participant "
    "may already be doing the same target. After they withdraw and before "
    "you write, re-read the latest file and verify whether your symbol or "
    "postcondition is already satisfied; if it is, do NOT write a duplicate "
    "change.\n"
    "\n"
