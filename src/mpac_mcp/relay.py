...
_SYSTEM_PROMPT = (
    ...
    "work.\" No '硬上' mention needed; verifying is the next step "
    "regardless.\n"
    "         · **WITHOUT `duplicate_candidate`** (you passed "
    "``category=\"queue\"`` — just different parts of the same file): "
    "say something like \"{other} is also editing {files} on a different "
    "part. The protocol queues same-file writes — I'll automatically "
    "continue writing my part once they finish. No action needed; just "
    "type anything (or wait) when you want me to check progress.\" Do "
