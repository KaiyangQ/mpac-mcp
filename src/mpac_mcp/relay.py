...
_SYSTEM_PROMPT = (
    ...
    "is working on them.\n"
    "     * **If overlap exists AND you are going to back off** (whether "
    "because the user told you to step aside, said 'don't do anything', "
    "'just check', 'wait', or because you decided yielding is safer): "
    "you MUST call **defer_intent(files, observed_intent_ids=[...])** "
    "with the intent_ids returned by check_overlap, BEFORE replying to "
    "the user. This is NOT optional. Without defer_intent, the user's "
    "collaborators see no signal that you yielded — the Conflicts panel "
    "stays empty and the whole MPAC coordination loop breaks. Saying "
    "'I'm yielding' / '我让路了' in chat without calling defer_intent "
    "is a protocol violation. After defer_intent succeeds, then tell "
    "the user you yielded and stop. Do NOT call announce_intent in "
    "this branch. The phrases 'do nothing' / 'don't do anything' / "
    "'什么都不要做' from the user mean 'don't ANNOUNCE or WRITE' — "
    "they NEVER mean 'skip defer_intent'; defer_intent is part of "
    "yielding, not part of doing.\n"
    "     * If overlap exists AND the user wants to proceed anyway: skip "
    "defer_intent and continue to step 3.\n"
    "  3. Call announce_intent(files, objective) BEFORE the first write. "
    "Remember the returned intent_id within THIS turn.\n"
