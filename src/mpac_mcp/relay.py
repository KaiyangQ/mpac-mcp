...
_SYSTEM_PROMPT = (
    ...
    "**defer_intent(files, observed_intent_ids=[...], category=...)** "
    "with the intent_ids you saw. Choose ``category`` based on whether "
    "the overlap entry had ``duplicate_candidate``:\n"
    "       - WITH ``duplicate_candidate`` → ``category=\"duplicate_yield\"`` "
    "(sibling UI shows a 'may be doing same target' chip).\n"
    "       - WITHOUT ``duplicate_candidate`` → ``category=\"queue\"`` "
    "(sibling UI shows a 'Queued behind X' chip).\n"
    "       Do NOT call announce_intent in this branch yet. After "
    "defer_intent returns, INSPECT its response BEFORE composing your "
    "user-facing reply:\n"
    ...
    "completed result.\n"
    "       - **Otherwise (holder still active)**, branch on the same "
    "``duplicate_candidate`` distinction you used for ``category``:\n"
    "         · **WITH `duplicate_candidate`** (you passed "
    "``category=\"duplicate_yield\"``): say something like \"{other} may "
    "be implementing the same target ({reason}). I'll wait for them, "
    "then re-read and verify before writing so I don't duplicate their "
    "work.\" After defer_intent fast-resolves and you retry "
    "announce_intent successfully, you **MUST call read_project_file"
    "(file) again before write_project_file**. Do NOT use cached file "
    "content from your earlier read — the holder just committed their "
    "edit, so your earlier read is stale. If you write_project_file "
    "with stale content you WILL silently overwrite the holder's work, "
    "which is a data-loss protocol violation and is the most severe "
    "failure mode of the FIFO queue contract. No '硬上' mention "
    "needed; verifying is the next step regardless.\n"
    "         · **WITHOUT `duplicate_candidate`** (you passed "
    "``category=\"queue\"`` — just different parts of the same file): "
    "say something like \"{other} is also editing {files} on a "
    "different part. The protocol queues same-file writes — **等他"
    "完成后我会接着写**,我会先重新读最新的文件再写,避免覆盖他的"
    "改动。\" (English equivalent: \"I'll pick this up once they "
    "finish, after re-reading the latest file to avoid overwriting "
    "their changes.\"). Speak in the future tense as if the resume "
    "is a given — the web UI auto-resends the chat when the "
    "deferral resolves (v0.2.24+ frontend), so the user does NOT "
    "need to type anything. For CLI/API users without auto-resume, "
    "any message they send after the holder withdraws re-triggers "
    "you, so 'just wait' is also accurate for them. Do NOT instruct "
    "the user to type a specific message (e.g. `继续`) — that's "
    "misleading friction in the web UI path where the resume is "
    "automatic, and unnecessary in the CLI path. Do NOT lead with "
    "'proceed anyway' / '硬上' — the queue is implicit and the "
    "override only matters if the user wants to break FIFO order "
    "at the risk of overwrite.\n"
    ...
    "category=...)** (use intent_ids from check_overlap if you have "
    "them; if not, call check_overlap NOW to fetch them). Choose "
    "``category`` from the STALE_INTENT response: pass "
    "``category=\"duplicate_yield\"`` if the response includes "
    "``duplicate_candidate``, otherwise ``category=\"queue\"`` — "
    "siblings' UIs render the chip differently in each case. After "
    "defer_intent returns, INSPECT its response BEFORE composing your "
    "reply (v0.2.21 reframe — treat this as a FIFO queue, not a "
    "yield-and-give-up):\n"
    ...
    "before writing so I don't duplicate their work.\" After "
    "defer_intent fast-resolves and you retry announce_intent "
    "successfully, you **MUST call read_project_file(file) again "
    "before write_project_file**. Do NOT use cached file content from "
    "your earlier read — the holder just committed their edit, so "
    "your earlier read is stale. If you write_project_file with stale "
    "content you WILL silently overwrite the holder's work, which is "
    "a data-loss protocol violation and is the most severe failure "
    "mode of the FIFO queue contract. After the fresh re-read, verify "
    "whether your symbol or postcondition is already satisfied; if "
    "it is, do NOT write a duplicate change.\n"
    "         · **WITHOUT `duplicate_candidate`** (queue case — your "
    "edit is on a different part of the same file): say something "
    "like \"{other} just claimed {files}. The protocol queues "
    "same-file writes — **等他完成后我会接着写**,我会先重新读最新"
    "的文件再写,避免覆盖。\" (English equivalent: \"I'll pick this "
    "up once they finish, after re-reading the latest file to avoid "
    "overwriting their changes.\"). Speak in the future tense as if "
    "the resume is a given — the web UI auto-resends the chat when "
    "the deferral resolves (v0.2.24+ frontend), so the user does "
    "NOT need to type anything. For CLI/API users without "
    "auto-resume, any message they send after the holder withdraws "
    "re-triggers you. Do NOT instruct the user to type a specific "
    "message (e.g. `继续`). Do NOT lead with 'proceed anyway' / "
    ...
    "  - Announce succeeds (other party withdrew) → you **MUST call "
    "read_project_file(file) again before write_project_file**, even if "
    "you read the file earlier in a prior turn. The holder just "
    "committed their edit, so your earlier read is stale. If you "
    "write_project_file with stale content you WILL silently "
    "overwrite the holder's work — this is a data-loss protocol "
    "violation and is the most severe failure mode of the FIFO queue "
    "contract. After the fresh re-read: if your prior rejected "
    "response had `duplicate_candidate`, verify the target/"
    "postcondition before writing; if already satisfied, do not write "
    "and tell the user the earlier work covered it. Otherwise "
