...
- **CONFLICTS 面板从头到尾空**(B 没 announce,server 端没创建 conflict —— 这是当前
  的设计;未来会加可视化 passive yield,届时这里会出现一张「B 让路」的 chip)
- db.py **没有** delete_all_notes

> **如果 B 没看到 A 的 intent**(check_overlap 返回空 → B 也 announce 了 → 面板出现冲突卡):
> 说明时机错了,A 已经 withdraw。两边 Reset to seed 重测,A 发完后再等更久再发 B,
> 或者把 A 的提示词再加 1-2 个函数让它更慢。
