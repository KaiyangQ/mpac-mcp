...
> 每个用例都按 `mpac 0.2.8` + `mpac-mcp 0.2.12` 的算法**精心设计**,
> 不靠运气、不靠时序。
>
> 文档包含**三类**测试:
>
...
> * **场景 7–9**(2026-04-29 新加)— 验证 mpac 0.2.8 race lock + mpac-mcp 0.2.11
>   分级默认值 + 0.2.12 announce response branches。
>
> **协议版本对应行为表**(对老 doc 的 review 时务必参考这个,因为同一个 prompt
> 在不同版本下行为不同):
>
> | 协议特性 | 引入版本 | 影响哪些用例 |
> |---|---|---|
> | scope_overlap advisory CONFLICT_REPORT | mpac 0.2.0 | 用例 1 / 6 |
> | dependency_breakage 文件级 | mpac 0.2.1 | 用例 2 / 5 |
> | symbol-precision (attr-chain + from-pkg-import) | mpac 0.2.3-0.2.4 | 用例 2 / 5 招牌帧 |
> | INTENT_DEFERRED 协议消息 + yield chip | mpac 0.2.5 / mpac-mcp 0.2.9 | 用例 4 |
> | 同 principal supersede(orphan cleanup)| mpac 0.2.6 | 隐含,所有重试 case |
> | Defer fast-resolve(observed-already-terminal)| mpac 0.2.7 | 用例 4 |
> | **scope_overlap race lock(STALE_INTENT)** | **mpac 0.2.8** | **场景 8 / 9** |
> | Prompt: defer_intent MUST on yield + 什么都不要做 carve-out | mpac-mcp 0.2.10 | 用例 4 |
> | Prompt: 分级默认值(scope_overlap yield / dep_breakage warn)| mpac-mcp 0.2.11 | 场景 7 |
> | Prompt: announce response branches(rejected → defer / conflicts → ⚠️)| mpac-mcp 0.2.12 | 场景 7 / 8 |
>
...
> **时机注意**:这个用例靠 A 持有 intent 的时间够长,B 来 check_overlap 时才看得到。
> 实测中如果 A 的任务太小,Claude 5-10 秒走完 announce → write → withdraw,B 才刚启动。
> 所以 A 的提示词**故意要做 2-3 件事**,把 intent 持有时间拉到 20-30 秒。

**A 粘到 chat:**

```
在 notes_app/db.py 里加 update_note(note_id, **fields) 和 patch_note(note_id, fields_dict) 两个函数 —— update_note 接 kwargs 形式参数，patch_note 接 dict 形式，两者都是找到对应笔记后按字段更新，找不到返回 False。先 read 文件看清 _STORE 结构再加。
```

**B 粘到 chat(等 5 秒左右,看到 A 的 chat 已经开始读文件再发):**
...
**期望看到**(v0.2.7+ 行为,4-29 验证):
- A 走完整流程,db.py 末尾出现 `update_note()` 和 `patch_note()`
- B 的 Claude 调 check_overlap → 看到 A 的 intent → **不 announce**,**调 `defer_intent`**
  (mpac-mcp 0.2.10 的 prompt 强化让"什么都不要做"不再被 Claude 误读成"skip
  defer_intent")→ chat 里回复"已让路"
- **CONFLICTS 面板出现一张灰色虚线 yield chip**(v0.2.5+ INTENT_DEFERRED),文案
  类似 *"Bob's Claude saw Alice's Claude editing notes_app/db.py — stepped back"*
- A withdraw 时 server 自动 emit `INTENT_DEFERRED status="resolved"`
  (v0.2.7+ fast-resolve)→ **chip 立即消失**(不等 60s TTL)
- db.py **没有** delete_all_notes

> **历史 baseline**(v0.2.4 / mpac-mcp 0.2.8 及更早):server 端不创建 conflict,
> 面板从头到尾空。这跟 v0.2.5+ 的 INTENT_DEFERRED yield chip 行为不同 ——
> 如果你看到面板**没**出现 chip,有 4 种可能:
> (a) Claude 没调 defer_intent —— 检查 mpac-mcp 是否 ≥ 0.2.10(prompt 强化)
> (b) Claude 调了 defer_intent,但 fast-resolve 太快(observed intent 已 terminal
>      时 active+resolved 同毫秒)→ 这是场景 9 描述的 UX 问题
> (c) check_overlap 返回空(time race 太短 / A 完成太快)→ Claude 直接 announce
>      变成场景 8 的 race lock 路径
> (d) 浏览器 cache 旧 frontend bundle → Cmd+Shift+R hard reload 重测
...
> **时机注意**:跟用例 4 一样,A 任务设计成 2 个函数让 intent 持有时间够长,
> B 几乎同时发(差距 ≤ 5 秒)就能撞上。如果还是没冲突,见用例 4 的 troubleshooting。

**A 粘到 chat:**

```
在 notes_app/db.py 末尾加 archive_note(note_id) 和 list_archived_notes() 两个函数 —— archive_note 把笔记从 _STORE 移到一个新的 _ARCHIVE 字典(不存在就创建)；list_archived_notes 返回所有归档笔记的 note_id 列表。如果遇到冲突，告诉我冲突内容但继续完成，不要让路。
```

**B 粘到 chat(同时,差距不超过 5 秒):**

```
在 notes_app/db.py 末尾加 unarchive_note(note_id) 和 archive_count() 两个函数 —— unarchive_note 把笔记从 _ARCHIVE 移回 _STORE；archive_count 返回 _ARCHIVE 的笔记数量。如果遇到冲突，告诉我冲突内容但继续完成，不要让路。
```
...
- 双方都 write 完文件
- **db.py 最后只有其中一边的函数** —— 第二个 write 覆盖了第一个(last-writer-wins),
  另外两个函数不见了

**这个用例是反面教材**:演示 MPAC 的协议是**软约束**(警告但不阻止)。实际产品里
**应该 respect Yield 按钮**,这个用例展示忽略冲突的代价(数据丢失)。

> **2026-04-29 更新**:**用例 6 在 mpac 0.2.8 之后行为改变了**。0.2.8 加了
> cross-principal same-file race lock —— 第二个 announce 会被 `STALE_INTENT`
> 直接 reject,**根本不会 last-writer-wins**。要看真正的"两边都不让路"行为
> 需要在 prompt 里加 "硬上" / "proceed anyway" 让 Claude 显式 override
> 0.2.11 的 default yield 行为。或者,**用 Test 6.x(0.2.7 baseline 模拟)
> 通过 demo_driver 直接发 envelope 绕开 race lock 来观察 last-writer-wins**。

---

# 🔒 race detection 场景(2026-04-29 新加,验证 0.2.8 + 0.2.11 + 0.2.12)

> **背景**:用例 4-6 在 0.2.7 时代覆盖了 "B 主动让路" / "招牌冲突" /
> "都不让路" 三种 single-shot 场景。但 2026-04-29 测试中暴露了**race window**:
> 两个 Claude 几乎同时启动时,**双方 check_overlap 时对方都还没 announce → 都
> 看不到对方 → 都 announce → fire conflict 但 Claude 已经在写文件,没机会反思**。
>
> mpac 0.2.8 + mpac-mcp 0.2.11/0.2.12 引入「分级锁」修复:
>
> | Conflict 类型 | 类比 git | 0.2.8+ 行为 |
> |---|---|---|
> | scope_overlap (同文件) | merge conflict | **协议层硬拒** announce(STALE_INTENT) |
> | dependency_breakage (跨文件) | semantic conflict | announce 接受 + 同步 conflicts 字段 → Claude ⚠️ warning |
>
> 下面 3 个场景就是测这套机制的。
>
> **重点提示**:race lock 测试**要让两个 announce 在几百毫秒到几秒内撞上**,
> 否则先到的已经 withdraw,race 窗口完全错过。Alice 的 prompt 必须**任务足够重**
> (read 多个文件 + 加多个函数 + docstring),把 intent 持有期拉到 30-60 秒。

---

## 场景 7 — Cross-file race(测 dependency_breakage 默认 PROCEED + ⚠️ warning)

**剧本**:Alice on `db.py`,Bob on `api.py`,**两边同时贴**(差距 ≤ 1 秒)。

**A 粘到 chat:**

```
把 notes_app/db.py 里的 save() 改成 save(note, *, dry_run=False) -> int —— 加一个 dry_run 关键字参数(默认 False)，返回保存的笔记数量。
```
...
**期望看到**:
- 双方都 announce 成功(不同文件,race lock 不触发)
- ⚡ 跳一张 `Dependency breakage` 红卡(招牌帧文案带 `notes_app.db.save`)
- **后到的那个 Claude(spoke 一方)的 chat 第一段以 ⚠️ 开头**,文案应该有
  类似:*"⚠️ Alice's Claude is changing `notes_app.db.save` in notes_app/db.py,
  which my work in notes_app/api.py depends on..."*  + 告诉用户"说 'wait for'/
  '让路'可以让我等她完成"
- 双方继续完成 → 文件都更新
...
```
T+0     Alice announce  db.py
T+1s    Bob   announce  api.py    same_tick_conflicts=1
T+1s    CONFLICT_REPORT cat=dependency_breakage
        dependency_detail.ba: [{file: api.py, symbols: [notes_app.db.save]}]
```

**为什么必触发**:Bob 是后到的,server 在他的 announce response 同步带 `conflicts:
[{category: dependency_breakage, ...}]` 字段。Bob 的 Claude 看 mpac-mcp 0.2.12
prompt 的"non-empty conflicts → ⚠️ warning"分支,prefix warning 给 user。

**Known limitation(architecture 限制)**:**先到的 Alice 看不到 conflict** ——
她 announce 时还没人撞,response 是空的。**这是 0.2.x 的 hard 限制**:Claude
子进程没有 reactive event subscription,relay 不会主动推 inbound CONFLICT_REPORT
给正在跑的 `claude -p`。Option C(0.3.x)解决。

**如果两边都没 ⚠️**:可能 Bob 也是先到,看不到 conflict。或者 Claude 没听 prompt
...
**剧本**:**两边都改 `db.py`,且任务足够重让两边 announce 撞在一起**。

**A 粘到 chat:**

```
在 notes_app/db.py 末尾加 5 个函数：archive_note(note_id), list_archived_notes(), restore_all_notes(), purge_archive(), get_archive_stats()。

每个函数都要先 read notes_app/db.py 和 notes_app/models.py 再写，并写完整的 docstring（Args、Returns、Raises 三段）。

功能：
- archive_note: 把笔记从 _STORE 移到一个新的 _ARCHIVE 字典(不存在就创建)
- list_archived_notes: 返回所有归档笔记的 note_id 列表
- restore_all_notes: 全量从 _ARCHIVE 移回 _STORE
- purge_archive: 清空 _ARCHIVE
- get_archive_stats: 返回 {count, oldest_id, newest_id}
```

**B 粘到 chat(同时,差距 ≤ 1 秒):**

```
在 notes_app/db.py 末尾加 unarchive_note(note_id) 和 archive_count() 两个函数 —— unarchive_note 把笔记从 _ARCHIVE 移回 _STORE；archive_count 返回 _ARCHIVE 的笔记数量。
```
...
- **后到的那一方(Bob)的 announce 被 server 409 reject(STALE_INTENT)**
  → Bob 的 Claude 调 `defer_intent` + chat 说类似:
  *"另一个参与者在改 db.py,我让路了,要 retry 请说 'proceed anyway' / '硬上'"*
- **CONFLICTS 面板出现 yield chip**(灰色虚线)
- **db.py 最后只有 Alice 的 5 个函数,Bob 的 2 个函数 NOT 写入**
- Alice 完成 + withdraw → yield chip 立即消失(fast-resolve)

**协议层证据**:

```
T+0      Alice announce  db.py     ← 注册成功
T+1s     Bob   announce  db.py     ← server 返回 PROTOCOL_ERROR(STALE_INTENT)
                                     description: "Files ['notes_app/db.py'] are already
                                     being modified by intent {alice's intent_id}..."
                                   ← Bob 的 announce_intent endpoint return HTTP 409
                                     with structured detail.
                                   ← Bob 的 announce_intent MCP tool catches 409,
                                     return {"rejected": true, "error_code":
                                     "STALE_INTENT", ...} dict (NOT exception).
T+2s     Bob   defer_intent       ← Claude 看 0.2.12 prompt 的 rejected branch
                                     调 defer_intent,observed_intent_ids=[alice's]
T+30s    Alice withdraw  reason=done
T+30s    INTENT_DEFERRED status=resolved (fast-resolve)
```

**为什么必触发**:Bob announce 时 Alice 还 active,coordinator
`_handle_intent_announce` 先做 cross-principal race detection
(`proposed_files & other_files` 不空)→ reject。Bob 的 mpac-mcp 0.2.12
`relay_tools.announce_intent` 正确处理 409 → return 结构化 dict → Claude 看
prompt 的 rejected branch → defer_intent。

**如果 race 没触发(Bob announce 成功 + 也写文件)**:Alice 任务太轻 / Bob 启动
太晚。重测时确保 A 的 prompt 真的让 Alice 干 30-60 秒,Bob 同时贴(差距 ≤ 1 秒)。

**override 测试**:跑完上面之后,再发 Bob 一句 `proceed anyway`(或 `硬上`)
→ Bob 的 Claude 应该重新尝试。如果 Alice 已经 withdraw,Bob 这次 announce 会成功。
如果 Alice 还在 active,Bob 会再次被 reject(然后告诉用户"她还没完成")。

---

## 场景 9 — Sequential same-file(测 0.2.11 default yield)

> 跟用例 4 类似,但简化版:Alice 先到,Bob 几秒后到。验证 mpac-mcp 0.2.11 的
> "scope_overlap default yield" prompt rule + mpac 0.2.7 fast-resolve。

**剧本**:Alice 先发,**等 5-8 秒**(确保她 announce 完),再发 Bob。
...
**B 粘到 chat(等 ~8 秒,看到 Alice 已经 announce):**

```
我想在 notes_app/db.py 里加一个 delete_all_notes() 清空 _STORE。
```

> **注意**:跟用例 4 的 prompt 不同 —— 这里**不**告诉 Bob "如果别人在改就放弃"。
> 完全自然 prompt,看 0.2.11 的 default yield prompt rule 是否在 LLM 行为层有效。

**期望看到**(v0.2.11+ 行为):
- Bob 的 Claude 调 `check_overlap` → 看到 Alice on db.py + category=`scope_overlap`
- **Bob 自动调 `defer_intent`(0.2.11 prompt 默认 yield)** → chat 说类似:
  *"Alice's Claude is editing the same file. I yielded — say 'proceed anyway' /
  '硬上' if you want me to continue despite the conflict."*
- **CONFLICTS 面板出现 yield chip**(灰色虚线)
- Alice 完成 + withdraw → chip 立即消失(fast-resolve)
- db.py **没有** `delete_all_notes`
...
```
T+0      Alice announce  db.py
T+8s     Bob   chat received
T+10s    Bob's Claude → check_overlap → 看到 Alice 的 scope_overlap entry
T+12s    Bob   INTENT_DEFERRED  observed_intent_ids=[alice's]  reason="..."
T+12s    INTENT_DEFERRED status=active broadcast
T+30s    Alice withdraw
T+30s    INTENT_DEFERRED status=resolved (fast-resolve)
```

**override 测试**:Bob defer 完之后,再发 `硬上` 或 `proceed anyway` → Bob 的
Claude 应该尝试 announce(如果 Alice 还活着,会被 race lock 拒;如果 Alice 已
withdraw,会成功)。

**Known UX issue**(2026-04-29 实测发现):如果 Alice 任务太短(9 秒就完成),
Bob defer 时 Alice 已经 terminal → fast-resolve 立即触发 → **active + resolved
同毫秒广播**→ yield chip 在 UI 上一闪而过,可能用户看不到。这是 fast-resolve
工作得太好的副作用,backlog: 加 minimum chip lifetime(前端 2-3s 最小显示)。

---

## race detection 场景一句话备忘

| 场景 | 触发什么 | 关键算法路径 |
|---|---|---|
| **场景 7** | dependency_breakage + ⚠️ warning(后到 spoke 看到)| announce response.conflicts(0.2.12) |
| **场景 8** | scope_overlap STALE_INTENT reject + defer chip | mpac 0.2.8 race lock + mpac-mcp 0.2.12 announce response branches |
| **场景 9** | scope_overlap default yield + chip + fast-resolve | mpac-mcp 0.2.11 split defaults + mpac 0.2.7 fast-resolve |

**3 个场景都覆盖的 backlog**:Claude 子进程缺 reactive event subscription —
先到的 announcer 永远看不到自己撞了别人,write reply 时也用 stale snapshot
("正在 vs 已经"问题)。Option C(0.3.x)真闭环。
...
| **用例 4** | **2** | `INTENT_DEFERRED`(被动让路)| check_overlap → defer_intent → yield chip(v0.2.5+ 行为;v0.2.4 baseline 不 fire) |
| **用例 5** | **2** | **`dependency_breakage`**(招牌)| 真改文件 + 符号级冲突文案,产品讲故事帧 |
| **用例 6** | **2** | **`scope_overlap`** + last-writer-wins | 软约束语义验证,反面教材(v0.2.8+ 被 race lock 替代,要 'proceed anyway' override) |
...
| **场景 7**(2026-04-29) | **2** | **`dependency_breakage` + ⚠️ warning** | mpac-mcp 0.2.12 announce response.conflicts → Claude prefix ⚠️(后到 spoke 看到)|
| **场景 8**(2026-04-29) | **2** | **`STALE_INTENT` reject + `INTENT_DEFERRED`** | mpac 0.2.8 race lock + mpac-mcp 0.2.12 rejected branch → defer_intent |
| **场景 9**(2026-04-29) | **2** | **`INTENT_DEFERRED`** | mpac-mcp 0.2.11 default yield + mpac 0.2.7 fast-resolve(自然 prompt 验证)|
