# MPAC v0.1.10 Developer Reference
...
| `version` | string | R | 协议版本，如 `"0.1.10"` |
...
MPAC v0.1.10 共 21 种消息类型，分布在 Session / Intent / Operation / Conflict / Governance / Error 六类中。
...
| 2 | (none) | COMMITTED | OP_COMMIT（post_commit） | state_ref_before 可验证 | 记录状态变更 |
...
| 6 | PROPOSED | REJECTED | OP_REJECT / 引用 intent 进入 `EXPIRED` / `WITHDRAWN` / `SUPERSEDED` / `TRANSFERRED` / frozen scope 超时 | | 通知发送者 |
...
| 12 | (none) | per-entry | OP_BATCH_COMMIT | post_commit: 直接提交；pre_commit: 首次提交进入 PROPOSED，授权后再次发送同 `batch_id` 完成声明 | scope = union of targets |
...
| 5 | ACKED | RESOLVED | RESOLUTION | sender 在当前 authority phase 有裁决权 | 执行 outcome |
| 6 | ESCALATED | RESOLVED | RESOLUTION | sender = escalate_to，或为 session policy 明确允许的 arbiter，或 coordinator 的系统裁决 | 执行 outcome，释放 frozen scope |
...
**并发裁决、claim 处置与活锁防护（v0.1.8+ / v0.1.10）：**
