# MPAC v0.1.8 Developer Reference
...
MPAC v0.1.8 共 20 种消息类型，分布在 4 个协议层中。
...
**Split-brain 防护**：参与者如果在同一 session 内收到来自两个不同 coordinator 的 `COORDINATOR_STATUS`，MUST 拒绝 Lamport 时钟值更低的 coordinator 的消息，SHOULD 向双方发送 `PROTOCOL_ERROR`（`error_code`: `COORDINATOR_CONFLICT`）。
