# MPAC v0.1.9 Developer Reference
...
- `message_id` 在 Authenticated profile 下用于重放检测，coordinator 会拒绝重复值；恢复后必须继续执行同一 replay-protection 策略
...
MPAC v0.1.9 共 21 种消息类型，分布在 Session / Intent / Operation / Conflict / Governance / Error 六类中。
...
| `approved_by` | string | O | 批准该 claim 的 principal ID | → [Principal](#11-principal参与者身份) |
...
**并发裁决规则**（Section 18.4）：同一 `conflict_id` 的多条 RESOLUTION，coordinator MUST 仅接受第一条有效裁决（按 coordinator 接收顺序），后续裁决以 `RESOLUTION_CONFLICT` 拒绝。与 INTENT_CLAIM 的 first-claim-wins 模式一致。
...
| 字段 | 类型 | 必填 | 说明 | 关联 |
|------|------|------|------|------|
| `event` | string | R | 枚举：`heartbeat` / `recovered` / `handover` / `assumed` | |
...
| `successor_coordinator_id` | string | O | `event` = `handover` 时必填：继任者 principal ID | → [Principal](#11-principal参与者身份) |
| `next_coordinator_epoch` | integer | O | `event` = `handover` 时必填：继任者将使用的 epoch | |
   ...
   └──────────────┘                             │         │
              ...
              INTENT_WITHDRAW│    TTL 过期  │  │  │ SUPERSEDED
                     ...
                     └──────────┘  └────────┘ │ └──────────────┘
                           ...
                           ┌─────────│  SUSPENDED    │──────────┐
                           │          └──────────────┘          │
                     参与者重连                  INTENT_CLAIM_STATUS approved
                           │                                    │
                           ▼                                    ▼
                   ┌──────────────┐                    ┌──────────────┐
                   │    ACTIVE    │                    │ TRANSFERRED  │
...
| SUSPENDED → TRANSFERRED | `INTENT_CLAIM_STATUS.decision = approved` | 原 intent 关闭，新 intent 激活 |
...
| 8 | SUSPENDED | TRANSFERRED | `INTENT_CLAIM_STATUS(approved)` | first-claim-wins，审批期内无原所有者重连 | 创建新 intent |
...
| PROPOSED → REJECTED | Reviewer 拒绝 / 引用 intent 进入 `EXPIRED` / `WITHDRAWN` / `SUPERSEDED` / `TRANSFERRED` / frozen scope 超时 |
...
| 5 | PROPOSED | REJECTED | OP_REJECT / 引用 intent 进入 `EXPIRED` / `WITHDRAWN` / `SUPERSEDED` / `TRANSFERRED` / frozen scope 超时 | | 通知发送者 |
| 6 | PROPOSED | ABANDONED | 发送者不可用 | 心跳超时 | |
| 7 | PROPOSED | FROZEN | 引用 intent 进入 SUSPENDED | | |
| 8 | FROZEN | PROPOSED | 引用 intent 恢复 ACTIVE | | |
| 9 | FROZEN | REJECTED | 引用 intent 从 SUSPENDED 进入 `EXPIRED` / `WITHDRAWN` / `SUPERSEDED` / `TRANSFERRED` | | |
...
| `governance` | core + INTENT_UPDATE, INTENT_WITHDRAW, INTENT_CLAIM, INTENT_CLAIM_STATUS, OP_PROPOSE, OP_REJECT, OP_SUPERSEDE, CONFLICT_ACK, CONFLICT_ESCALATE | 必须指定 arbiter；intent-before-action 为 MUST；推荐 pre_commit 模型；progressive degradation |
...
同一 `conflict_id` 的多条 RESOLUTION → coordinator MUST 仅接受第一条有效裁决（按 coordinator 接收顺序），后续以 `RESOLUTION_CONFLICT` 拒绝。

- 适用于所有角色组合（多 owner、多 arbiter、混合角色）
...
**并发裁决、claim 处置与活锁防护（v0.1.8+ / v0.1.9）：**
- [ ] Coordinator 对同一 conflict_id 仅接受第一条 RESOLUTION，后续以 `RESOLUTION_CONFLICT` 拒绝
