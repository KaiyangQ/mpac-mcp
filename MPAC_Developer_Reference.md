# MPAC v0.1.7 Developer Reference
...
MPAC v0.1.7 共 20 种消息类型，分布在 4 个协议层中。
...
| `execution_model` | string | R | 枚举：`pre_commit` / `post_commit`。声明 session 的执行模型 | → [执行模型](#91-执行模型) |
...
| 字段 | 类型 | 必填 | 说明 | 关联 |
|------|------|------|------|------|
| `reason` | string | R | 枚举：`manual` / `all_complete` / `session_ttl` / `coordinator_shutdown` | |
| `summary` | object | O | `{ total_intents, completed_intents, total_operations, committed_operations, total_conflicts, resolved_conflicts }` | |
| `transcript_uri` | string | O | 会话记录的导出 URI | |
| `active_intents` | string[] | O | 关闭时仍活跃的 intent ID | → INTENT_ANNOUNCE 的 `intent_id` |
| `active_conflicts` | string[] | O | 关闭时仍未解决的 conflict ID | → CONFLICT_REPORT 的 `conflict_id` |

**副作用**：收到后参与者 MUST 停止发送业务消息。后续消息收到 `SESSION_CLOSED` 错误。

---

### 3.19 COORDINATOR_STATUS

Coordinator 心跳，携带状态快照用于故障恢复。

| 字段 | 类型 | 必填 | 说明 | 关联 |
|------|------|------|------|------|
| `status` | string | R | 枚举：`healthy` / `degraded` / `shutting_down` | |
| `uptime_sec` | integer | R | Coordinator 运行时间（秒） | |
| `lamport_clock` | integer | R | 当前 Lamport 时钟值 | → [Watermark](#14-watermark因果水位) |
| `participant_count` | integer | R | 当前活跃参与者数 | |
| `active_intents` | integer | R | 活跃 intent 数 | |
| `active_conflicts` | integer | R | 未解决 conflict 数 | |
| `snapshot` | object | O | 状态快照（JSON），用于故障恢复。包含完整内部状态 | |
| `snapshot_watermark` | Watermark | O | 快照时的 Lamport 值，用于恢复时确定 audit log 回放起点 | → [Watermark](#14-watermark因果水位) |

**频率**：SHOULD 定期发送。参与者可用 `lamport_clock` 检测 coordinator 是否落后（split-brain 防护）。

---

### 3.20 OP_BATCH_COMMIT

原子多目标操作。将多个 OP_COMMIT 打包为一个原子批次。
...
**原子性语义**：`all_or_nothing` 模式下任一操作失败则全部回滚；`best_effort` 模式下尽量多提交。Scope = 所有 target 的并集。

**Pre-commit 模型**：coordinator 收到后先做 scope 检查和冲突检测，确认后再通知参与者执行。
...
| SESSION_CLOSE.`active_intents[]` | → INTENT_ANNOUNCE.`intent_id` | 关闭时活跃的 intent |
| SESSION_CLOSE.`active_conflicts[]` | → CONFLICT_REPORT.`conflict_id` | 关闭时未解决的冲突 |
| COORDINATOR_STATUS.`snapshot_watermark` | → Watermark | 快照时因果水位 |
...
|---|---------|---------|-------------|---------|------|
| 1 | (none) | ANNOUNCED | INTENT_ANNOUNCE | sender 已完成 HELLO 握手 | 注册 intent，启动 TTL 计时器 |
| 2 | ANNOUNCED | ACTIVE | Coordinator 接受 | 无冲突或冲突已声明 | 通知所有参与者 |
| 3 | ACTIVE | EXPIRED | TTL 过期 | coordinator 墙钟 ≥ received_at + ttl_sec | 触发 Expiry Cascade |
| 4 | ACTIVE | WITHDRAWN | INTENT_WITHDRAW | sender = intent owner | 触发 Expiry Cascade |
| 5 | ACTIVE | SUPERSEDED | 新 intent.supersedes_intent_id 匹配 | 新 intent 已被接受 | 触发 Expiry Cascade |
| 6 | ACTIVE | SUSPENDED | 不可用检测 | 心跳超时 ≥ unavailability_timeout_sec | 关联 PROPOSED → FROZEN |
| 7 | SUSPENDED | ACTIVE | 所有者重连 | HELLO + 身份验证通过 | FROZEN → PROPOSED |
| 8 | SUSPENDED | TRANSFERRED | INTENT_CLAIM 审批 | first-claim-wins，审批期内无原所有者重连 | 创建新 intent |
| 9 | SUSPENDED | EXPIRED | 超时/外部决策 | | 触发 Expiry Cascade |
...
| # | 起始状态 | 目标状态 | 触发消息/事件 | 守卫条件 | 动作 |
|---|---------|---------|-------------|---------|------|
| 1 | (none) | PROPOSED | OP_PROPOSE | intent 存在且 ACTIVE（Governance profile） | 注册操作 |
| 2 | PROPOSED | COMMITTED (post_commit) | OP_COMMIT | state_ref_before 可验证 | 记录状态变更 |
| 3 | PROPOSED | CONFIRMED (pre_commit) | Coordinator 确认 | scope 检查通过，无冲突 | 通知参与者执行 |
| 4 | CONFIRMED | COMMITTED | 参与者报告完成 | 变更已应用 | 记录 state_ref_after |
| 5 | PROPOSED | REJECTED | OP_REJECT / intent 终态 / frozen scope 超时 | | 通知发送者 |
| 6 | PROPOSED | ABANDONED | 发送者不可用 | 心跳超时 | |
| 7 | PROPOSED | FROZEN | 引用 intent 进入 SUSPENDED | | |
| 8 | FROZEN | PROPOSED | 引用 intent 恢复 ACTIVE | | |
| 9 | FROZEN | REJECTED | 引用 intent 从 SUSPENDED 进入终态 | | |
| 10 | COMMITTED → SUPERSEDED | OP_SUPERSEDE | 被替代 op 必须为 COMMITTED | 记录 supersession chain |
| 11 | (none) | per-entry | OP_BATCH_COMMIT | all_or_nothing: 全部或回滚；best_effort: 逐项 | scope = union of targets |
...
| 2 | OPEN | ACKED | CONFLICT_ACK | sender 是关联实体的 owner | |
| 3 | OPEN | ESCALATED | CONFLICT_ESCALATE | escalate_to 是有效 arbiter | 通知 arbiter |
| 4 | OPEN | ESCALATED | 自动升级（Phase 2） | frozen scope 持续 > phase_1_sec | 触发优先级旁路 |
| 5 | ACKED | RESOLVED | RESOLUTION | sender 有 owner/arbiter 角色 | 执行 outcome |
| 6 | ESCALATED | RESOLVED | RESOLUTION | sender = escalate_to 或有 arbiter 角色 | 执行 outcome，释放 frozen scope |
| 7 | RESOLVED | CLOSED | 裁决执行完毕 | outcome 中所有动作已执行 | |
| 8 | OPEN | DISMISSED | 所有关联实体终结 | auto-dismiss 条件满足 | 释放 frozen scope |
| 9 | ESCALATED | DISMISSED | 所有关联实体终结 | auto-dismiss 条件满足 | 释放 frozen scope |
| 10 | ACKED | DISMISSED | 所有关联实体终结 | auto-dismiss 条件满足 | 释放 frozen scope |
| 11 | ESCALATED | first-committer-wins | Phase 3 降级 | frozen scope 持续 > phase_1 + phase_2 | 按 Lamport 时钟排序，首提交者获胜 |
...
| 值 | 必须支持的消息类型 | 额外要求 |
|----|--------------------|---------|
| `core` | HELLO, SESSION_INFO, SESSION_CLOSE, COORDINATOR_STATUS, GOODBYE, HEARTBEAT, INTENT_ANNOUNCE, OP_COMMIT, OP_BATCH_COMMIT, CONFLICT_REPORT, RESOLUTION, PROTOCOL_ERROR | Lamport clock 规则、一致性模型语义 |
| `governance` | core + INTENT_UPDATE, INTENT_WITHDRAW, INTENT_CLAIM, OP_PROPOSE, OP_REJECT, OP_SUPERSEDE, CONFLICT_ACK, CONFLICT_ESCALATE | 必须指定 arbiter；intent-before-action 为 MUST；推荐 pre_commit 模型；progressive degradation |
...
|------|---------------|---------|
| `pre_commit` | OP_PROPOSE → coordinator 确认 → 参与者执行 → OP_COMMIT | 高协调场景，写前检查 |
| `post_commit` | 参与者先执行变更 → OP_COMMIT 为事后声明 | 低延迟场景，事后协调 |

**Pre-commit 流程**：OP_PROPOSE → coordinator 做 scope 检查 + 冲突检测 → 确认 → 参与者执行 → OP_COMMIT。
...
| # | 规则 | 说明 |
|---|------|------|
| 1 | 初始化 | 所有参与者和 coordinator 初始值为 0 |
| 2 | 发送规则 | 发送消息前 clock++ |
...
| 6 | 单调性 | Lamport 值 MUST 单调递增，MUST NOT 回退 |
...
| Phase 3 | > (phase_1 + phase_2)（默认 300s+） | first-committer-wins：按 Lamport 时钟排序，首提交者获胜 |
...
- [ ] Lamport clock 遵循 6 条维护规则（Section 12.7）
...
**执行模型：**
- [ ] Pre-commit: OP_PROPOSE → coordinator 确认 → 执行 → OP_COMMIT
...
**Coordinator 故障恢复：**
- [ ] 定期发送 COORDINATOR_STATUS（含 snapshot）
- [ ] 支持 snapshot + audit log replay 恢复
- [ ] Split-brain 检测（Lamport clock 比较）
