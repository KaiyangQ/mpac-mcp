# MPAC v0.1.12 Developer Reference
...
| `version` | string | R | 协议版本，如 `"0.1.12"` |
...
MPAC v0.1.12 共 21 种消息类型，分布在 Session / Intent / Operation / Conflict / Governance / Error 六类中。
...
| 字段 | 类型 | 必填 | 说明 | 关联 |
|------|------|------|------|------|
| `event` | string | R | 枚举：`heartbeat` / `recovered` / `handover` / `assumed` / `authorization` | |
